document.addEventListener('DOMContentLoaded', () => {
    ...
    // Páginas donde NO aparece el modal de registro al hacer clic en "Agregar"
    // y SÍ aparece el carrito flotante
    const paginasConCarrito = ['pagina.php', 'dia_de_la_madre.php', 'dia_del_padre.php', 'dia_del_maestro.php'];
    const pathActual = window.location.pathname.toLowerCase();
    const esConCarrito = paginasConCarrito.some(p => pathActual.includes(p));

    if (!esConCarrito) {
        // En index.html y otras páginas: mostrar modal de registro al agregar
    ...
    // --- Carrito Flotante (en pagina.php y páginas de eventos) ---
    if (esConCarrito) {
        const floatingCart = document.getElementById('floating-cart');
        const floatingBadge = document.getElementById('floating-cart-badge');
        let floatingCartCount = 0;

        if (floatingCart && floatingBadge) {
            botones.forEach(boton => {
                boton.addEventListener('click', () => {
                    floatingCartCount++;
                    floatingBadge.textContent = floatingCartCount;
                    floatingBadge.classList.add('visible');

                    // Animación pop en el carrito flotante
                    ...
                    showLuxuryToast('🛍️ <span>¡Producto añadido al <strong>carrito</strong>!</span>');
                });
            });
        }
    }
